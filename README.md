...
一个围绕电子、硬件、超频实验与次元收藏展开的个人博客。首页以 RhineLabUI 的全屏终端构图为主要设计：斜向三维档案阵列、固定品牌与工具、底部编号和分类导航。选中的档案从阵列中抽出，镜头接续到左侧档案与右侧概览；全文和收藏图集保持独立地址。

## 主要功能

- 白昼与夜间双主题，带有从切换按钮向全屏扩散的过渡动画
- 五类档案循环切换，支持方向键、滚轮、触摸滑动与全站索引搜索
- 抽取、归位、镜头和列轨道使用保留速度的临界阻尼，连续输入接续当前运动
- 三维阵列批量渲染，只有选中档案使用完整玻璃材质；静止后停止绘制
- 手机端使用紧凑终端布局和可滚动概览，不支持 WebGL 时提供可操作档案列表
- 中英双语界面，文字通过分解、溃散和重组完成语言切换；后台保存中文时自动生成英文内容
- 双主题指针、轻量尾迹、全局点击反馈与可中断的档案切换
...
| 样式与动效 | CSS 动画、Three.js 实例化阵列、按需绘制 |
