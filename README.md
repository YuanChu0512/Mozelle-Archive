...
一个围绕电子、硬件、超频实验与次元收藏展开的个人博客。主站使用伊蕾娜白昼、Mon3tr / 罗德岛夜间双主题，保留人物首屏、技术文章卡片、实验笔记与次元收藏分区。导航采用按真实尺寸生成的液态玻璃折射，全文和图集保持独立地址。

## 主要功能

- 白昼与夜间双主题，带有从切换按钮向全屏扩散的过渡动画
- 技术文章分类筛选、独立实验笔记、收藏图集和全站搜索
- 液态玻璃按导航尺寸与圆角生成边缘位移图，中心保持平直，内部文字不参与折射
- 导航只用一层背景滤镜，嵌套按钮共享背景；指针高光离开后在原位置淡出
- 移动端与降低透明度模式使用轻量材质，避免大面积滤镜开销
- 中英双语界面，文字通过分解、溃散和重组完成语言切换；后台保存中文时自动生成英文内容
- 双主题指针、轻量尾迹、全局点击反馈、法阵与球体动效
...
| 样式与动效 | CSS 动画、Canvas、局部 SVG 背景折射 |
...
三维阵列终端保存为备用版本：[backup-rhine-terminal-2026-09-09](https://github.com/YuanChu0512/Mozelle-Archive/tree/backup-rhine-terminal-2026-09-09)。备用代码保留在版本历史中，不作为当前主站入口加载。
...
```bash
git clone https://github.com/YuanChu0512/Mozelle-Archive.git
cd Mozelle-Archive
cp .env.example .env
```
